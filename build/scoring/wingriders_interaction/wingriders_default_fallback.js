...
async function calcW1(other) {
    if (!other.length)
        return [0, undefined];
    const hasWingriders = other.find(({ role }) => role.toUpperCase().includes("WINGRIDERS"));
    if (hasWingriders)
...
async function calcW2(metadata) {
    ...
    return [_1.util.weighMetadataMsg("674", ["WingRiders"], metadata) * weighting.metadata, undefined];
}
