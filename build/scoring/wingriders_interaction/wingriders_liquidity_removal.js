...
async function score(intermediaryTx, bfAddressInfo, lucidAddressDetails, txInfo, txUTXOs) {
    ...
    if (scTotal?.length) {
        const tokens = scTotal.filter(({ currency }) => !currency.includes("-LPT-")).map(({ currency, amount }) => _1.util.formatAmount(-amount, currency));
        const lps = scTotal.filter(({ currency }) => currency.includes("-LPT-")).map(({ currency }) => currency.replace("WR-LPT-", "").replaceAll("/", "-"));
        const description = `Withdrew ${_1.util.joinWords(tokens)} from ${_1.util.joinWords(lps)} ${lps.length > 1 ? "pools" : "pool"} on Wingriders`;
...
async function calcW1(user) {
    ...
    return [weighting.userAccounts * Math.min(Object.keys(assets)
            .filter((currency) => assets[currency] > 0).length, 2) / 2, undefined];
}
/**
...
async function calcW2(other) {
    if (!other.length)
        return [0, undefined];
    let score = 0;
    const wingridersRequestAddress = other.find(({ role }) => role === "Wingriders Request");
    if (wingridersRequestAddress)
        score += 1;
    const lpt = wingridersRequestAddress?.total.find(({ currency }) => currency.includes("-LPT-"));
    if (lpt)
        score += 1;
    const wingridersScriptAddress = other.find(({ role, total }) => role !== "Unknown Address" && total.find(({ currency, amount }) => currency === lpt?.currency && -amount === lpt?.amount));
