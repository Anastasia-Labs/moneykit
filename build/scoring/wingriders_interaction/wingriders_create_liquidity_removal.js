...
async function calcW1(user) {
    ...
    if (!currencies.length)
        return [0, undefined];
    const pairCount = currencies.filter((currency) => (currency.includes("-LPT-") && assets[currency] < 0) // LPT must be negative
        || currency === "ADA").length;
    const pairTokens = currencies.find((currency) => {
...
async function calcW2(other) {
    ...
    for (const { role, total } of other) {
        const wingridersScriptAddress = role.startsWith("Wingriders")
            && total.length === 2
            && total.every(({ currency, amount }) => (currency.startsWith("WR-LPT") || currency === "ADA") &&
                amount > 0);
        if (wingridersScriptAddress)
            depositLptAddressCount += 1;
    }
...
async function calcW3(metadata) {
    ...
    return [_1.util.weighMetadataMsg("674", "WingRiders liquidity".split(" "), metadata) * weighting.metadata, undefined];
}
