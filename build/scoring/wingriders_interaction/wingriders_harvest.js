...
async function score(intermediaryTx, bfAddressInfo, lucidAddressDetails, txInfo, txUTXOs) {
    ...
    const description = harvestedTokens?.length
        ? `Harvested ${_1.util.joinWords(harvestedTokens)} from Wingriders`
...
async function calcW1(other) {
    const assets = other.reduce((sum, { role, total }) => {
    ...
    if (!currencies.length)
        return [0, undefined];
    const harvestedTokens = currencies.filter((currency) => assets[currency] > 0 && !currency.includes("-LPT-")).map((currency) => _1.util.formatAmount(assets[currency], currency));
