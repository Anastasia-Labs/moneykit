...
async function score(intermediaryTx, bfAddressInfo, lucidAddressDetails, txInfo, txUTXOs) {
    ...
    try {
        ...
        const fromToken = _1.util.formatAmount(qty, `${otherTokens[lpToken]} LP Token`);
        const toTokens = Object.keys(receivedTokens)
            .map((currency) => {
            let qty = receivedTokens[currency];
            if (currency === "ADA")
                qty -= 2; // adjustment based on what's displayed on Minswap's dashboard
            ...
            return _1.util.formatAmount(qty, currency);
        });
        const description = `Zapped-out ${fromToken} for ${_1.util.joinWords(toTokens)} on Minswap`;
...
async function calcW1(user) {
    ...
    for (const { address, total } of user) {
        const { paymentCredential, stakeCredential } = await _1.lucid.getAddressDetails(address);
...
async function calcW2(other, txUTXOs) {
    ...
    for (const { address, role, total } of other) {
        try {
            if (role === "Unknown Address")
                continue; // skip non-script address
            for (const { currency, amount } of total) {
                try {
                    const nonLP = !(currency.startsWith("asset") && currency.length === 44);
                    if (nonLP || amount < 0)
                        continue; // skip non-LP or negative amount
                    for (const output of txUTXOs.outputs) {
                        try {
                            if (output.address !== address || !output.data_hash)
                                continue; // skip
                            const { json_value } = await _1.bf.getDatum(output.data_hash);
                            lpNames[`${currency}-${amount}`] = await _1.lucid.toText(json_value?.fields[2].fields[1].bytes);
                        }
                        catch {
                            continue;
                        }
