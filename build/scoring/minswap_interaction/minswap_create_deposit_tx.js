...
async function score(intermediaryTx, bfAddressInfo, lucidAddressDetails, txInfo, txUTXOs) {
    ...
    const description = depositCurrencies.length
        ? `Created a deposit request of ${_1.util.joinWords(depositCurrencies)} on Minswap`
...
async function calcW1(user) {
    ...
    if (scriptTotalLength > 2)
        delete scriptTotal.ADA;
...
async function calcW2(metadata) {
    ...
    return [_1.util.weighMetadataMsg("674", "Minswap Deposit Order".split(" "), metadata) * weighting.metadata, undefined];
}
