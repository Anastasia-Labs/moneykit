...
async function calcW2(other, txUTXOs) {
    if (!other.length)
        return [0, undefined];
    let stakedToken = undefined;
    for (const { address, role } of other) {
        try {
            if (role.startsWith("Minswap Yield Farming")) {
                const utxo = txUTXOs.outputs.find((input) => input.address === address);
                if (!utxo?.data_hash)
                    continue;
                const { json_value } = await _1.bf.getDatum(utxo.data_hash);
                stakedToken = await _1.lucid.toText(json_value.fields[3].list[0].fields[0].fields[1].bytes);
                if (stakedToken)
                    break;
            }
        }
...
async function calcW3(metadata) {
    ...
    return [_1.util.weighMetadataMsg("674", "Minswap Stake liquidity".split(" "), metadata) * weighting.metadata, undefined];
}
