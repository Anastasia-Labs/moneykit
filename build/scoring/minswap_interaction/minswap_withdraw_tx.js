...
async function score(intermediaryTx, bfAddressInfo, lucidAddressDetails, txInfo, txUTXOs) {
    ...
    if (receivedTokensFromLP?.length === 2) {
        ...
        const withdrewTokens = Object.keys(receivedTokens)
            .map((currency) => _1.util.formatAmount(receivedTokens[currency] - (currency === "ADA" ? 2 : 0), currency));
        const description = `Withdrew ${_1.util.joinWords(withdrewTokens)} from ${fromLP.slice(0, fromLP.length - 3)} pool on Minswap`;
        ...
        try {
            ...
            if (paymentCredential?.type === "Script" || stakeCredential?.type === "Script") {
                for (const { currency, amount } of account.total) {
                    ...
                    if (nonLP || amount > 0)
                        continue; // skip NonLP Tokens or positive amounts
