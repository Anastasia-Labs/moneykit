 ...
 * @returns [Score, AdditionalData]
 */
async function calcW3(metadata, txInfo) {
    if (!metadata.length)
        return [0, undefined];
    const label6862 = metadata.filter(({ label }) => label === "6862");
    if (!label6862.length)
        return [0, undefined];
    const withPools = label6862.find(({ json_metadata }) => json_metadata?.pools?.length > 1);
