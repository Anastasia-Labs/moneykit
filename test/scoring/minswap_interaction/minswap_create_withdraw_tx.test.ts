...
import { score } from "../../../src/scoring/minswap_interaction/minswap_create_withdraw_tx";
...
describe("minswap_create_withdraw_tx", () => {
  beforeEach(() => {
    vi.clearAllMocks();
    vi.clearAllTimers();
  });
  ...
  it("score: 1", async () => {
    //#region Arrange
    const createWithdrawTxScore: TransactionScore = {
      type: "amm_dex",
      description: "Created a withdraw ADA-MIN LP order on Minswap",
      score: 1,
    };
    //#endregion
    ...
    //#region Assert
    expect(transactionScore).toStrictEqual(createWithdrawTxScore);
    //#endregion
  });
  ...
  it("score: .5", async () => {
    ...
    const createWithdrawTxScore: TransactionScore = {
      type: "amm_dex",
      description: "Created a withdraw order on Minswap",
      score: .5,
    };
    //#endregion
