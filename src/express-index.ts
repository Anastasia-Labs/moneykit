...
const dailyLimit = rateLimit({
  ...
  handler:
    (_, rsp) =>
      rsp.status(429).json({ error: "Daily limit reached!" }),
});
const burstLimit = rateLimit({
  ...
  handler:
    (_, rsp) =>
      rsp.status(429).json({ error: "Burst limit reached!" }),
});
...
const expressOpenAPI: openapi.ExpressOpenAPIArgs = {
  ...
  apiDoc: {
    info: {
      ...
      license: {
        name: "MIT",
        url: "https://raw.githubusercontent.com/Anastasia-Labs/moneykit/refs/heads/main/LICENSE",
      },
    },
    openapi: "3.0.4",
    servers: [
...
if (!env.HOSTED) file.writeFile(
  ...
  (error) =>
    error ? Logger.log.error(error) : undefined,
);
